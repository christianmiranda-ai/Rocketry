...
delta_H = delta_p / g_0 # change in fluid head equation (m)
...
r_eye = (Q / (math.pi * omega *phi_e)*(1 - ((r_inner**2) A (r_eye**2)))) # eye radius equation (m) -- use MATLAB solver?
r_exit = ((1/omega)*math.sqrt(g_0*delta_H/psi)) # exit radius equation (m)
...
beta_outlet = math.atan((Q / (2*math.pi*r_exit*w_exit)) / (omega*r_exit*(1 - mu) - ((g_0*delta_p) / (eta_HY * omega * r_exit))))
...
delta_h_e = delta_h_es + delta_h_ev # total head change equation (m)